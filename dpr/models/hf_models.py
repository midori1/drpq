...
def get_bert_biencoder_components(args, inference_only: bool = False, **kwargs):
    ...
    from .cluster_models import ClusterBertEncoder, FirstBertEncoder
    ...
    if args.clustering:
        biencoder = ClusterBertEncoder(question_encoder, ctx_encoder, fix_ctx_encoder=fix_ctx_encoder,
                                       n_cluster=args.n_cluster, random_select=args.random_init_cluster,
                                       use_cls=args.use_cls, use_roberta=False, time=args.time)
    elif args.first:
        biencoder = FirstBertEncoder(question_encoder, ctx_encoder, fix_ctx_encoder=fix_ctx_encoder,
                                       n_cluster=args.n_cluster, random_select=args.random_init_cluster,
                                       use_cls=args.use_cls, use_roberta=False, time=args.time)
...
def get_roberta_biencoder_components(args, inference_only: bool = False, **kwargs):
    ...
    if args.clustering:
        biencoder = ClusterBertEncoder(question_encoder, ctx_encoder, fix_ctx_encoder=fix_ctx_encoder,
                                       n_cluster=args.n_cluster, random_select=args.random_init_cluster,
                                       use_cls=args.use_cls, use_roberta=True, time=args.time)
    elif args.first:
        biencoder = FirstBertEncoder(question_encoder, ctx_encoder, fix_ctx_encoder=fix_ctx_encoder,
                                       n_cluster=args.n_cluster, random_select=args.random_init_cluster,
                                       use_cls=args.use_cls, use_roberta=True, time=args.time)
...
def get_bert_tensorizer(args, tokenizer=None):
    if not tokenizer:
        if args.init_from_path:
            ...
            tokenizer = get_bert_tokenizer(args.vocab_path, do_lower_case=args.do_lower_case)
...
def get_roberta_tensorizer(args, tokenizer=None):
    if not tokenizer:
        if args.init_from_path:
            assert args.vocab_path is not None
            tokenizer = get_roberta_tokenizer(args.vocab_path, do_lower_case=args.do_lower_case)
...
class HFBertEncoder(BertModel):
    ...
    def init_encoder(cls, cfg_name: str, projection_dim: int = 0, dropout: float = 0.1, **kwargs) -> BertModel:
        if 'init_from_path' in kwargs and kwargs['init_from_path']:
            model_path = kwargs['model_path']
            cfg_path = kwargs['config_path']  # include .json
            cfg = BertConfig.from_pretrained(cfg_path)
            del kwargs['init_from_path']
            del kwargs['model_path']
            del kwargs['config_path']
            return cls.from_pretrained(model_path, config=cfg, project_dim=projection_dim, **kwargs)
        else:
            if 'init_from_path' in kwargs:
                del kwargs['init_from_path']
                del kwargs['model_path']
                del kwargs['config_path']
...
class HFRobertaEncoder(RobertaForSequenceClassification):

    def __init__(self, config, project_dim: int = 0):
        RobertaForSequenceClassification.__init__(self, config)
        assert config.hidden_size > 0, 'Encoder hidden_size can\'t be zero'
        self.embeddingHead = nn.Linear(config.hidden_size, project_dim) if project_dim != 0 else None
    ...
    def init_encoder(cls, cfg_name: str, projection_dim: int = 0, dropout: float = 0.1, **kwargs) -> BertModel:
        if 'init_from_path' in kwargs and kwargs['init_from_path']:
            model_path = kwargs['model_path']
            cfg_path = kwargs['config_path']  # include .json
            cfg = RobertaConfig.from_pretrained(cfg_path)
            del kwargs['init_from_path']
            del kwargs['model_path']
            del kwargs['config_path']
            model, info = cls.from_pretrained(model_path, config=cfg, project_dim=projection_dim,
    ...
    def forward(self, input_ids: T, token_type_ids: T, attention_mask: T) -> Tuple[T, ...]:
        if self.config.output_hidden_states:
            sequence_output, pooled_output, hidden_states = self.roberta(input_ids=input_ids,
                                                                         token_type_ids=token_type_ids,
                                                                         attention_mask=attention_mask)
        else:
            hidden_states = None
        ...
        pooled_output = sequence_output[:, 0, :]
        if self.embeddingHead:
            pooled_output = self.norm(self.embeddingHead(pooled_output))
        return sequence_output, pooled_output, hidden_states

    def get_out_size(self):
        if self.embeddingHead:
            return self.embeddingHead.out_features
        return self.config.hidden_size
...
class BertTensorizer(Tensorizer):
    ...
    def text_to_tensor(self, text: str, title: str = None, add_special_tokens: bool = True, max_length=None):
        text = text.strip()
