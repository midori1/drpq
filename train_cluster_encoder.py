...
from dpr.models.cluster_models import ClusterBertEncoder, ClusterNllLoss
...
class BiEncoderTrainer(object):
    ...
    def get_dict_data_iterator(self, psgs_path, queries_path, qrels_path, trec_path,
                               batch_size: int, shuffle=True, shuffle_seed: int = 0,
                               offset: int = 0, split=True) -> ShardedDataIterator:
        psgs_data = self.get_psgs(psgs_path)
        queries = self.get_queries(queries_path)
        qrels = self.get_qrels(qrels_path)
        trec = self.get_trec(trec_path)

        data = []
        for qid, query in tqdm(queries.items(), desc='Generating data...'):
            positive_docids = qrels[qid].keys()
            if qid not in trec:
                continue
            trec[qid] = [docid for docid, rank in trec[qid]]
            negative_docids = set(trec[qid]) - positive_docids
            question = query
            positive_ctxs = [{'title': psgs_data[docid]['title'],
                              'text': psgs_data[docid]['text'],
                              'url': psgs_data[docid]['url'],
                              'psg_id': docid} for docid in positive_docids]

            negative_ctxs = [{'title': psgs_data[docid]['title'],
                              'text': psgs_data[docid]['text'],
                              'url': psgs_data[docid]['url'],
                              'psg_id': docid} for docid in negative_docids]
            data.append({
                'question': question,
                'positive_ctxs': positive_ctxs,
                'negative_ctxs': [],
                'hard_negative_ctxs': negative_ctxs  # 当前没有使用negative_ctxs，所以只赋值hard_negative_ctxs
            })
        ...
        if not split:
            ...
            return ShardedDataIterator(data, shard_id=0,
                                       num_shards=1,
                                       batch_size=batch_size, shuffle=shuffle, shuffle_seed=shuffle_seed, offset=offset,
                                       strict_batch_size=True,
                                       )

    def get_psgs(self, path):
        psgs_data = {}
        with open(path, 'r') as f:
            for line in tqdm(f, desc='loading documents'):
                docid, url, title, content = line.strip('\n').split('\t')
                url = url.rstrip()
                title = title.rstrip()
                content = content.rstrip()
                # truncate
                content = ' '.join(content.split(' ')[:self.args.sequence_length])
                psgs_data[docid] = {'title': title, 'text': content, 'url': url}
        return psgs_data
        ...
        with open(path, 'r') as f:
            for line in f:
                ...
                if line[0] not in qrels:
                    qrels[line[0]] = {}
                qrels[line[0]][line[2]] = line[3]
        return qrels

    def get_trec(self, path):
        trec = {}
        with open(path, 'r') as f:
            for line in f:
                line = line.strip('\n').split()
                if len(line) > 3:
                    qid, docid, rank = line[0], line[2], line[3]
                else:
                    qid, docid, rank = line[0], line[1], line[2]
                if qid not in trec:
                    trec[qid] = []
                trec[qid].append((docid, rank))
        return trec
    ...
    def run_train(self, ):
        ...
        if args.use_dict_input:
            train_iterator = self.get_dict_data_iterator(args.psgs_file, args.train_queries_file,
                                                         args.train_qrels_file, args.train_trec_file,
                                                         args.batch_size,
                                                         shuffle=True,
        ...
        self.validate_and_save(0, 0, None)
    ...
    def validate_nll(self) -> float:
        ...
        num_hard_negatives = args.dev_hard_negatives
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_data()):
            biencoder_input = ClusterBertEncoder.create_biencoder_input(samples_batch, self.tensorizer,
                                                                        ...
                                                                        shuffle=False)
            loss, correct_cnt, return_info = _do_biencoder_fwd_pass(self.biencoder, biencoder_input, self.tensorizer,
                                                                    args,
                                                                    return_mrr=True)
            mrr = return_info['mrr']
        ...
        logger.info('NLL Validation: MRR@10 = %f. loss = %f. correct prediction ratio  %d/%d ~  %f',
                    MRR,
                    total_loss,
    ...
    def _train_epoch(self, scheduler, epoch: int, eval_step: int,
        ...
        num_hard_negatives = args.train_hard_negatives
        num_other_negatives = args.other_negatives
        seed = args.seed
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_data(epoch=epoch)):
            ...
            biencoder_batch = ClusterBertEncoder.create_biencoder_input(samples_batch, self.tensorizer,
                                                                        True,
                                                                        max_query_length=args.query_sequence_length,
                                                                        max_doc_length=args.sequence_length,
                                                                        num_hard_negatives=num_hard_negatives,
            ...
            loss, correct_cnt, return_info = _do_biencoder_fwd_pass(self.biencoder, biencoder_batch, self.tensorizer, args)
            ...
            if i % log_result_step == 0:
                ...
                if self.args.local_rank <= 0:
                    ...
                    cur_loss = logging_loss / log_result_step
                    self.tb_writer.add_scalar('train/loss', cur_loss, self.global_step)
                logger.info(
...
def _calc_loss(args, loss_function, local_q_vector, local_ctx_vectors, local_positive_idxs,
    ...
    if not return_scores:
        loss, is_correct = loss_function.calc(global_q_vector, global_ctxs_vector, positive_idx_per_question,
                                              hard_negatives_per_question, return_scores)
        return loss, is_correct
    else:
        loss, is_correct, scores = loss_function.calc(global_q_vector, global_ctxs_vector, positive_idx_per_question,
                                                      hard_negatives_per_question, return_scores)
        return loss, is_correct, scores


def _do_biencoder_fwd_pass(model: nn.Module, input: BiEncoderBatch, tensorizer: Tensorizer, args, return_mrr=False) -> (
        torch.Tensor, int):
    ...
    loss_function = ClusterNllLoss()
    if return_mrr:
        loss, is_correct, scores = _calc_loss(args, loss_function, local_q_vector, local_ctx_vectors, input.is_positive,
                                              input.hard_negatives, return_scores=return_mrr)
    else:
    ...
    if return_mrr:
        ...
        if scores.shape[1] < 10:
            ...
            topk = 10
        topk_scores, topk_idx = torch.topk(scores, dim=1, k=topk)
        for line, pos_idx in zip(topk_idx, input.is_positive):
    ...
    return loss, is_correct, return_info
...
def main():
    ...
    parser.add_argument("--global_loss_buf_sz", type=int, default=10000000,
    ...
    parser.add_argument("--train_hard_negatives", default=1, type=int,
                        help="amount of hard negative ctx per question")
    parser.add_argument("--dev_hard_negatives", default=10, type=int,
                        help="amount of hard negative ctx per question")
    ...
    # path of input files
    parser.add_argument("--use_dict_input", action='store_true', help='whether the inputs are in dict type')
    parser.add_argument("--psgs_file", default=None, type=str)

    parser.add_argument("--train_queries_file", default=None, type=str)
    parser.add_argument("--train_qrels_file", default=None, type=str)
    parser.add_argument("--train_trec_file", default=None, type=str,
                        help='two formats are valid:'
                             'qid\tdocid\trank, example: "1185869	D59222	1". '
                             'or'
                             'qid\t"Q0"\tdocid\trank\tscore\trunstring which is same as "msmarco-doctrain-top100"')

    parser.add_argument("--dev_queries_file", default=None, type=str)
    parser.add_argument("--dev_qrels_file", default=None, type=str)
    parser.add_argument("--dev_trec_file", default=None, type=str)
    parser.add_argument("--dev_psgs_file", default=None, type=str)
    ...
    args.log_dir = f"{args.log_dir}/{time_stamp}"
    ...
    # set input file for different process
    # if args.local_rank != -1:
    #     args.train_qrels_file = f"{args.train_qrels_file}.{args.local_rank + 1}"
    #     args.train_trec_file = f"{args.train_trec_file}.{args.local_rank + 1}"
    #     args.train_queries_file = f"{args.train_queries_file}.{args.local_rank + 1}"
    #     args.psgs_file = f"{args.psgs_file}.{args.local_rank + 1}"

    trainer = BiEncoderTrainer(args)

    if args.train_file is not None or args.use_dict_input:
        trainer.run_train()
